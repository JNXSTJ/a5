...
def question_1h_sanity_check(model):
    ...
    sentence_length = 10
    max_word_length = 21
    inpt = torch.zeros(sentence_length, BATCH_SIZE,
                       max_word_length, dtype=torch.long)
...
def question_2a_sanity_check(decoder, char_vocab):
    ...
    logits_expected_size = [sequence_length,
                            BATCH_SIZE, len(char_vocab.char2id)]
...
def question_2c_sanity_check(decoder):
    ...
    assert(len(decodedWords) == BATCH_SIZE), "Length of decodedWords should be {} but is: {}".format(
